...
pub async fn handle_dispatch(
    ...
    tx.send(SQLiteCommand::Dispatch {
        reply: reply_tx,
    }).await?;
...
pub fn do_dispatch(conn: &mut Connection) -> Result<DispatchedJob> {
    ...
    let mut start_task = tx.prepare_cached("UPDATE tasks SET started_at = coalesce(started_at, CURRENT_TIMESTAMP) WHERE id = ?;")?;
    let mut start_job = tx.prepare_cached("UPDATE jobs SET started_at = CURRENT_TIMESTAMP WHERE id = ?;")?;
    ...
    let (job_id, task_id, job, args) = get_job.query_row([], |row| {
        ...
        let job = Job {
            inputs: serde_json::from_str(&inputs).map_err(|e| rusqlite::Error::ToSqlConversionFailure(Box::new(e)))?,
            output: PathBuf::from(output),
        };

        let args: Vec<Arg> = serde_json::from_str(&row.get::<_, String>(4)?).map_err(|e| rusqlite::Error::ToSqlConversionFailure(Box::new(e)))?;
